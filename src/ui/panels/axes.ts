...
export function buildDOMAxes(rect: DOMRect, renderer: any): void {
  const W = rect.width, H = rect.height;
  const view = renderer.fullViewTile(state);
  const { q1, q2 } = renderer.computeSliceDirs(state);

  function axisTitle(q: number[], topN: number = 2): string {
    const comps = q
  ...
  function zValueAt(t: number, axis: "h" | "v"): number {
    ...
    const z = state.z0.map((z0i, i) => z0i + uu * q1[i] + vv * q2[i]) as number[];
    return dot10(z as any, q);
  }
