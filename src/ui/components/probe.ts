...
export function uvFromClientXY(
  clientX: number,
  clientY: number,
  glCanvas: HTMLCanvasElement,
  outCanvas: HTMLCanvasElement
): { u: number; v: number; rect: DOMRect } {
...
export function zAtUV(u: number, v: number, renderer: any): number[] {
...
export function showProbeAtEvent(
  e: PointerEvent,
  probeTooltip: any,
  glCanvas: HTMLCanvasElement,
  outCanvas: HTMLCanvasElement,
  ...
  const showHudCheckbox = $("showHud") as HTMLInputElement | null;
  ...
  const p = decodeICParamsFromZ(z as any);
  ...
  const now = Date.now();
  if (!(showProbeAtEvent as any).lastSampleTime || now - (showProbeAtEvent as any).lastSampleTime > 200) {
    (showProbeAtEvent as any).lastSampleTime = now;
  ...
  const lines = [
    ...
    { type: "row", label: "m",     val: `[${p.m.map((x: number) => x.toFixed(5)).join(", ")}]` },
    { type: "row", label: "α, β",  val: `${p.alpha.toFixed(5)}, ${p.beta.toFixed(5)}` },
    { type: "row", label: "pρ",    val: `[${p.pRho.map((x: number) => x.toFixed(5)).join(", ")}]` },
    { type: "row", label: "pλ",    val: `[${p.pLam.map((x: number) => x.toFixed(5)).join(", ")}]` },
