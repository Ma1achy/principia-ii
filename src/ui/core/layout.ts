/**
 * Bounding box result from layout calculation
 */
export interface TitleBoundingBox {
 ...
 * 
 * @returns Bounding box constraints for title positioning
 */
export function computeTitleBoundingBox(): TitleBoundingBox {
  ...
  const outCanvas = document.getElementById("outCanvas") as HTMLCanvasElement | null;
  ...
  const obstacles: Rect[] = [
    plotEl ? inflateRect(rectOf(plotEl)!, CANVAS_PAD) : null,
    sidebar ? (rectOf(sidebar) ? inflateRect(rectOf(sidebar)!, SIDEBAR_PAD) : null) : null,
    axisLeft ? (rectOf(axisLeft) ? inflateRect(rectOf(axisLeft)!, AXIS_PAD) : null) : null,
    axisBot ? (rectOf(axisBot) ? inflateRect(rectOf(axisBot)!, AXIS_PAD) : null) : null,
    rectOf(hudEl),
    rectOf(legendEl),
  ].filter((r): r is Rect => r !== null);
  
  // Ray vs AABB intersection
  function rayHitAABB(ox: number, oy: number, dx: number, dy: number, r: Rect): RayHit | null {
  ...
  // Cast diagonal ray
  function diagonalBounds(ox: number, oy: number): RayHit {
    ...
    let best: RayHit = { t: len, x: vw, y: vh };
