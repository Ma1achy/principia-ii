...
/**
 * Interrupt urgency level type
 */
export type InterruptUrgencyLevel = typeof INTERRUPT_URGENCY[keyof typeof INTERRUPT_URGENCY];

/**
 * Maps event types to urgency levels
 */
export const EVENT_URGENCY_MAP: Record<string, InterruptUrgencyLevel> = {
 ...
 * @returns Urgency level (0-3)
 */
export function getEventUrgency(eventType: string): InterruptUrgencyLevel {
 ...
 * @param eventType - Event type name
 * @param urgency - Urgency level (0-3)
 * @returns Priority level (1-3)
 */
export function getEventPriority(eventType: string, urgency: number): number {
