/**
 * Text animation types and interfaces
 */
...
interface TypingParams {
  ...
  pauseChance?: number;
  pauseDuration?: SpeedRange | number;
}
...
interface TypoCorrectionTimings {
  realizePause: number;
  hesitationPause: number;
  retypeDuration: number;
  postFixPause: number;
}
...
interface DeletionParams {
  ...
  dasCharCount?: number;
  arrRate?: number;
  initialDeleteDelay?: number;
  chucklePause?: number;
  wordDeleteChance?: number;
  sentenceDeleteChance?: number;
}

interface DeletionOptions {
  emotion?: string;
  intensity?: number;
  clearStrategy?: 'backspace' | 'instant';
...
function isMathContext(index: number, chars: string[]): boolean {
...
function isStrongMathContext(index: number, chars: string[]): boolean {
  const char = chars[index];
  const prev = index > 0 ? chars[index - 1] : '';
  const next = index < chars.length - 1 ? chars[index + 1] : '';
...
function getMathTypo(char: string): string {
...
function getQwertyTypo(char: string): string {
...
function getTypoForIndex(index: number, chars: string[]): string {
  const char = chars[index];
...
class AnimationMomentum {
   ...
   */
  update(targetMult: number): number {
    const now = Date.now();
...
function getTypingParams(emotion: string, intensity: number): TypingParams {
  const params = {
    baseSpeed: 50,
    speedVariation: 0.3,
    typoChance: 0.03,
 ...
 */
function applyToneModifiers(params: TypingParams, tone: string, emotion: string): TypingParams {
  ...
  return {
    ...
    pauseDuration: (typeof params.pauseDuration === 'number' ? params.pauseDuration : (params.pauseDuration?.min || 0)) * pauseMult
  };
}
...
function isClausePunctuation(char: string): boolean {
  return /[,;:]/.test(char);
}
...
function isShiftLikelyChar(char: string): boolean {
  // Uppercase letters and common shifted symbols
  return /[A-Z!@#$%^&*()_+{}|:"<>?]/.test(char);
}

function isRareSymbolChar(char: string): boolean {
  return /[\[\]{}<>\\|`~]/.test(char);
}

function isUnicodeOrAccented(char: string): boolean {
  // crude but useful: non-ascii
  return /[^\x00-\x7F]/.test(char);
}

function isMathOrGreekSymbol(char: string): boolean {
 ...
 */
function dampedIntensity(intensity: number, curve: number = 2.0): number {
 ...
 */
function getCharacterComplexity(char: string): number {
 ...
 */
function validateAndClampDuration(duration: any): number {
  const MIN_PAUSE = 50;
 ...
 */
function validateAndMergePauses(segments: TextSegment[]): TextSegment[] {
  const merged = [];
  let i = 0;
 ...
 */
function extractPauseMarkers(text: string): TextSegment[] {
  const segments = [];
 ...
 */
function cleanPauseMarkers(text: string): string {
 ...
 */
function buildPauseMap(segments: TextSegment[]): Map<number, number> {
 ...
 */
function sampleRange(range: SpeedRange | undefined): number {
...
export function animateTextInTyping(
  element: HTMLElement,
  targetText: string,
  onComplete: () => void,
  ...
  function getHumanizedStepDelay(index) {
    ...
    if (Math.random() < effectivePauseChance) {
      ...
      const pauseDur = typeof typingParams.pauseDuration === 'number' ? typingParams.pauseDuration : (typingParams.pauseDuration?.min || 0);
      delay += pauseDur * jitter;
    }
...
function getDeletionParams(emotion: string, intensity: number): DeletionParams {
  // Damped intensity to avoid extremes
  const dampedI = dampedIntensity(intensity, 2.0);
  ...
  switch (emotion) {
    ...
    case 'AMUSED':
      ...
      chucklePause = intensity > 0.5 ? 500 : 0;
  ...
  // Base params (from emotion)
  const baseSpeed = dasDelay;
  const speedVariation = arrRate;
...
export function animateTextOut(
  ...
  try {
    Array.from(element.childNodes).forEach(node => {
      if (node.nodeType === Node.ELEMENT_NODE && (node as HTMLElement).tagName === 'SPAN' && !(node as HTMLElement).classList.contains('text-cursor')) {
        charSpans.push(node);
      }
    });
  } catch (error) {
 ...
 */
function traceSpeedCalculation(emotion: string, intensity: number, tone: string): any {
  const trace = [];
 ...
 */
  if (typeof window !== 'undefined') {
    ...
    (window as any).chazyDebug.testSpeed = function(config: any) {
    ...
    (window as any).chazyDebug.speedBounds = SPEED_BOUNDS;
    (window as any).chazyDebug.idleBounds = IDLE_BOUNDS;
    (window as any).chazyDebug.pauseBounds = PAUSE_BOUNDS;
    (window as any).chazyDebug.displayBounds = DISPLAY_BOUNDS;
    
    (window as any).chazyDebug.resetMomentum = function() {
