 ...
 * Field order matches the WGSL definition; indexing tests verify the
 * round-trip from the GPU buffer.
 */
export interface TileReduction {
  id:                       TileID;
  level:                    number;
  mean_n_checkpoints:       readonly { x: number; y: number; z: number; w: number }[];
  mean_arc_length_n:        number;
  mean_t_end:               number;
  mean_d_min:               number;
  mean_ftle:                number;
  mean_energy_drift:        number;
  mean_diffusion:           number;
  spread_n:                 number;
  spread_arc_length_n:      number;
  spread_t_end:             number;
  spread_d_min:             number;
  spread_ftle:              number;
  spread_energy_drift:      number;
  spread_diffusion:         number;
  outcome_impurity:         number;
  dominant_outcome:         number;
  suspect_fraction:         number;
  suspect_lz_fraction:      number;
  energy_drift_worst:       number;
  lz_drift_worst:           number;
  mean_word_length:         number;
  spread_word_length:       number;
  word_agreement:           number;
  dominant_word_hash:       number;
  ensemble_outcome_agreement: number;
  ensemble_count:           number;
  mean_orbit_count:         number;
  retrograde_fraction:      number;
  coherence_score:          number;          // GPU writes S_tile here
  priority_score:           number;          // CPU writes priority here
  sample_count:             number;
  status_flags:             number;
}
...
function decodeTileReduction(ab: ArrayBuffer, M: number): TileReduction {
  ...
  return {
    ...
    mean_t_end:            f32[base + 1],
    mean_d_min:            f32[base + 2],
    mean_ftle:             f32[base + 3],
    mean_energy_drift:     f32[base + 4],
    mean_diffusion:        f32[base + 5],
    spread_n:              f32[base + 6],
    spread_arc_length_n:   f32[base + 7],
    spread_t_end:          f32[base + 8],
    spread_d_min:          f32[base + 9],
    spread_ftle:           f32[base + 10],
    spread_energy_drift:   f32[base + 11],
    spread_diffusion:      f32[base + 12],
    outcome_impurity:      f32[base + 13],
    dominant_outcome:      u32[base + 14],
    suspect_fraction:      f32[base + 15],
    suspect_lz_fraction:   f32[base + 16],
    energy_drift_worst:    f32[base + 17],
    lz_drift_worst:        f32[base + 18],
    mean_word_length:      f32[base + 19],
    spread_word_length:    f32[base + 20],
    word_agreement:        f32[base + 21],
    dominant_word_hash:    u32[base + 22],
    ensemble_outcome_agreement: f32[base + 23],
    ensemble_count:        i32[base + 24],
    mean_orbit_count:      f32[base + 25],
    retrograde_fraction:   f32[base + 26],
    coherence_score:       f32[base + 27],
    priority_score:        f32[base + 28],
    sample_count:          i32[base + 29],
